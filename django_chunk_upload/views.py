...
class ChunkUploadBaseView(View):
    """
    Base view for the rest of chunk upload views.
    """
    ...
    def get_queryset(self, request):
        """
        Get (and filter) ChunkUpload queryset.
    ...
    def validate(self, request):
        ...
        Must raise ChunkUploadError if validation fails.
        """
    ...
    def pre_save(self, chunk_upload, request, new=False):
        """
    ...
    def save(self, chunk_upload, request, new=False):
        """
    ...
    def post_save(self, chunk_upload, request, new=False):
        """
        Placeholder method for calling after saving an object.
        """

    def _save(self, chunk_upload):
        """
        Wraps save() method.
        """
        new = chunk_upload.id is None
        self.pre_save(chunk_upload, self.request, new=new)
        self.save(chunk_upload, self.request, new=new)
        self.post_save(chunk_upload, self.request, new=new)
...
class ChunkUploadView(ChunkUploadBaseView):
    """
    ...
    def get_extra_attrs(self, request):
        """
        Extra attribute values to be passed to the new ChunkUpload instance.
    ...
    def create_chunk_upload(self, save=False, **attrs):
        """
        Creates new chunk upload instance. Called if no 'upload_id' is
        found in the POST data.
        """
        chunk_upload = self.model(**attrs)
        # file starts empty
        chunk_upload.file.save(name='', content=ContentFile(''), save=save)
        return chunk_upload

    def is_valid_chunk_upload(self, chunk_upload):
        """
        Check if chunk upload has already expired or is already complete.
        """
        if chunk_upload.expired:
            raise ChunkUploadError(status=http_status.HTTP_410_GONE,
                                   detail='Upload has expired')
        error_msg = 'Upload has already been marked as "%s"'
        if chunk_upload.status == COMPLETE:
            raise ChunkUploadError(status=http_status.HTTP_400_BAD_REQUEST,
    ...
    def get_response_data(self, chunk_upload, request):
        ...
        return {
            ...
            'offset': chunk_upload.offset,
            'expires': chunk_upload.expires_on
        }
    ...
    def _post(self, request, *args, **kwargs):
        ...
        if upload_id:
            chunk_upload = get_object_or_404(self.get_queryset(request),
                                             upload_id=upload_id)
            self.is_valid_chunk_upload(chunk_upload)
        else:
            attrs = {'filename': chunk.name}
            if hasattr(request, 'user') and is_authenticated(request.user):
                attrs['user'] = request.user
            attrs.update(self.get_extra_attrs(request))
            chunk_upload = self.create_chunk_upload(save=False, **attrs)
        ...
        chunk_upload.append_chunk(chunk, chunk_size=chunk_size, save=False)

        self._save(chunk_upload)

        return Response(self.get_response_data(chunk_upload, request),
                        status=http_status.HTTP_200_OK)


class ChunkUploadCompleteView(ChunkUploadBaseView):
    """
    Completes an chunk upload. Method `on_completion` is a placeholder to
    ...
    def is_valid_chunk_upload(self, chunk_upload):
        """
        Check if chunk upload is already complete.
        """
        if chunk_upload.status == COMPLETE:
            error_msg = "Upload has already been marked as complete"
            return ChunkUploadError(status=http_status.HTTP_400_BAD_REQUEST,
                                    detail=error_msg)

    def md5_check(self, chunk_upload, md5):
        """
        Verify if md5 checksum sent by client matches generated md5.
        """
        if chunk_upload.md5 != md5:
            raise ChunkUploadError(status=http_status.HTTP_400_BAD_REQUEST,
    ...
    def _post(self, request, *args, **kwargs):
        ...
        self.is_valid_chunk_upload(chunk_upload)
        if self.do_md5_check:
            self.md5_check(chunk_upload, md5)

        chunk_upload.status = COMPLETE
        chunk_upload.completed_on = timezone.now()
        self._save(chunk_upload)
        self.on_completion(chunk_upload.get_uploaded_file(), request)
